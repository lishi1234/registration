function image_fusion1(image_1,image_2,solution)
...
solution_1=[1,0,N1;0,1,M1;0,0,1];%平移效果
tform=maketform('affine',solution_1');
f_1=imtransform(image_1,tform, 'XYScale',1,'XData',[1 3*N1], 'YData',[1 3*M1]);
tform=maketform('affine',(solution_1*solution)');
f_2=imtransform(image_2,tform, 'XYScale',1,'XData',[1 3*N1], 'YData',[1 3*M1]);

%%
same_index=find(f_1 & f_2);%Same region 均含有数据的位置
...
fusion_image(index_1)=f_1(index_1);% 每张图中单独的地方
fusion_image(index_2)=f_2(index_2);
fusion_image=uint8(fusion_image); % 获得匹配结果

% Delete redundant areas 删除冗余部分
