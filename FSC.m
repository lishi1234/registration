...
for i=1:1:iterations
    while(1)
        a=floor(1+(M-1)*rand(1,n));%（1~3）之间的随机数
        cor11=cor1(a,1:2);% 随机选取点作为内点
        cor22=cor2(a,1:2);
    ...
    [parameters,~]=LSM(cor11,cor22,change_form);% 最小二乘法求解变换矩阵
    ...
    if(strcmp(change_form,'perspective'))
        ...
        diff_match2_xy=t_match1_xy-match2_xy;%误差
        diff_match2_xy=sqrt(sum(diff_match2_xy.^2));
        index_in=find(diff_match2_xy<error_t);%误差小于3个像素
        consensus_num=size(index_in,2);
    end
     
    if(consensus_num>most_consensus_number)
        most_consensus_number=consensus_num;%更新内点数目，若大于则换新数据，小于则保留当前结果
        cor1_new=cor1(index_in,:);
        cor2_new=cor2(index_in,:);
    end
end
 
%Delete duplicate point pair 误匹配剔除后重新计算变换矩阵
uni1=cor1_new(:,[1 2]);
